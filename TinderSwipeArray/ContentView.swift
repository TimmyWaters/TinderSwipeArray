...
//  TinderSwipeArray
//
//  Created by Timothy Waters on 7/20/19.
...
struct ContentView : View {
    @State var cards = Array(repeating: NewCard(), count: 5)
    @State var x: Int = -5
    @State var y: Int = -10
    ...
    var body: some View {
        ZStack {
            Image("plant")
                .resizable()
                ZStack {
                    ForEach((0...cards.count-1).reversed()) { number in
                        self.cards[number].offset(x: CGFloat(self.x * number), y: CGFloat(self.y * number))
                    }
                }
        }
    }
